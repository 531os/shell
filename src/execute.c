...
SimpleCmd* handleSimpleCmdStr(int begin, int end){
    ...
    char c, buff[10][400], inputFile[30], outputFile[30], *temp = NULL;
        ...
        if(!pipei(buff[i]))
               {
                glob_t  test;
                test.gl_offs=0;
                glob(buff[i],GLOB_DOOFFS,NULL,&test);
                t=test.gl_pathc;
                for( t2=k-1;t2>i;t2--)
                strcpy(buff[t2+t],buff[t2]);
                for( t1=0;t1<t;t1++)
                strcpy(buff[i+t1],test.gl_pathv[t1]);
                for( t1=0;t1<t;t1++)
                {
                 j = strlen(buff[i+t1]);
                cmd->args[i+t1] = (char*)malloc(sizeof(char) * (j + 1));
                strcpy(cmd->args[i+t1], buff[i+t1]);
                }
                k=t+k-1;
                i=i+t-1;
                }
